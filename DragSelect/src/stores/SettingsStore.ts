...
export type DSSettings<E extends DSInputElement> = WithPostfix<
  Required<Settings<E>>,
  ':pre'
>

export type DSSettingsPublishEventNames =
  | 'Settings:updated:pre'
  | 'Settings:updated'
  | `Settings:updated:${string}:pre`
...
export default class SettingsStore<E extends DSInputElement> {
   ...
   * @ignore
   */
  constructor({ PS, settings }: { PS: PubSub<E>; settings: Settings<E> }) {
    this.PS = PS
  ...
  private _update = ({
    ...
    for (const [key, value] of Object.entries(_settings) as [
      keyof Settings<E>,
      DSSettings<E>[keyof Settings<E>],
    ][]) {
